...
function debugJumpToLevel(targetLevel) {
    if (!DEBUG_MODE)
        return;
    const currentLevel = getLevel();
    if (targetLevel <= currentLevel || targetLevel > levelConfig.maxLevel)
        return;
...
function drawDebugPanel() {
    if (!DEBUG_MODE || !debugPanelVisible)
        return;
    _buildButtons();
    const fs = fontScale;
...
function handleDebugClick(clickX, clickY) {
    if (!DEBUG_MODE || !debugPanelVisible)
        return false;
    const panelX = 4;
    const panelY = 4;
...
// 处理开关按钮点击
function handleDebugToggleClick(clickX, clickY) {
    if (!DEBUG_MODE || debugPanelVisible)
        return false;
    if (!debugToggleArea)
        return false;
