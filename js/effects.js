// 战斗特效模块：得分动效（+X 上浮）与伤害浮动动效（-X 上浮、暴击金色样式）
...
const SCORE_EFFECT_DURATION_MS = 1500; // 原 30 帧 × 50ms
...
class DamageEffectObj {
    constructor(x, y, damage, fontSize, color, floatDistance, durationMs, crit = false) {
        this.init(x, y, damage, fontSize, color, floatDistance, durationMs, crit);
    }
    // 重置全部状态（对象池复用入口，构造函数也走这里保证两条路径一致）
    init(x, y, damage, fontSize, color, floatDistance, durationMs, crit = false) {
        this.x = x;
        this.y = y;
        this.damage = damage;
...
function addDamageEffect(x, y, damage, fontSize, color, floatDistance, durationMs, stackOffset, crit = false) {
...
export { addScoreEffect, drawScoreEffects, clearScoreEffects, addDamageEffect, drawDamageEffects, clearDamageEffects };
