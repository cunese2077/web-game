...
function drawGameOver() {
    ...
    if (build.length > 0) {
        ...
        // 绘制一组条目（左侧标题+右侧条目列表）
        function drawGroup(title, entries, startY) {
            if (entries.length === 0)
                return startY;
...
let pendingDeleteRecordId = null; // null=删除全部, number=删除指定记录
let currentPage = 1;
const PAGE_SIZE = 10;
// Tooltip 系统
let mouseX = -1;
...
let gameDataHitAreas = [];
function isGameDataOpen() {
    return gameDataOpen;
}
...
function drawGameData() {
    ...
    for (const ach of achDefs) {
        ...
        const tierLabels = ["achievement.tier.bronze", "achievement.tier.silver", "achievement.tier.gold"];
    ...
    if (allRecords.length > 0) {
        ...
        if (!deleteConfirmVisible) {
            ...
            ctx.font = `bold ${Math.round(14 * fontScale)}px arial`;
            ctx.textAlign = "center";
            ctx.fillText(pendingDeleteRecordId !== null ? t("gameData.deleteRecordConfirm") : t("gameData.deleteAllConfirm"), cx, dlgY + Math.round(52 * fontScale));
            // 按钮区域
    ...
    if (tooltipVisible) {
        ...
        if (tipX + tipW > width)
            tipX = width - tipW - Math.round(4 * fontScale);
        if (tipY < 0)
            tipY = tooltipY + Math.round(16 * fontScale);
...
// 处理游戏数据页面的点击事件
function handleGameDataClick(clickX, clickY) {
