...
class Boss {
    ...
    update() {
        ...
        const isPatrolSuppressed = (this.bossType === "assault" || (this.bossType === "shifter" && this.stance === 0)) && this.isDiving;
    ...
    _updateShifter() {
        // 姿态切换闪光倒计时
        if (this.stanceFlashMs > 0)
            this.stanceFlashMs -= getDt();
        // 姿态计时累积，满 8 秒切换下一姿态
    ...
    _firePatternShifter(fanCount, aimedCount, bulletCfg) {
        if (this.stance === 0) {
