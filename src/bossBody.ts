...
interface BossBodyState {
  ...
  spiralAngle: number;
  // 变形型
  stance: number;        // 当前姿态（0=突击 1=堡垒 2=幻影）
  stanceFlashMs: number; // 姿态切换闪光剩余（ms，用于颜色渐变插值）
}
...
function drawShifterBody(b: BossBodyState, left: number, top: number): void {
    // 当前姿态核心色；切换闪光期间从上一姿态色渐变到新姿态色
    const prevStance = (b.stance + 2) % 3;
    let coreColor: string = SHIFTER_STANCE_COLORS[b.stance];
