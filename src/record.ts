...
function tryUpdateHighScore(_score: number): boolean {
  // 实际更新已在 recordGameEnd 中完成，这里返回比较结果
  return false;
}

// 尝试更新记录，返回是否刷新了最高等级
function tryUpdateHighLevel(_level: number): boolean {
  return false;
}

// 重置最高分和最高等级记录
// 现在由 resetAllData 统一管理，此处保留接口兼容
function resetHighScoreRecords(): void {
