...
let evolutionFlashMs: number = 0;
const EVOLUTION_FLASH_DURATION_MS: number = 1500; // 1.5秒

// BOSS 击败慢动作效果（ms 帧率无关）
let bossDefeatSlowMoMs: number = 0;
let bossDefeatX: number = 0;
let bossDefeatY: number = 0;

// ========== 中断续玩 ==========
// 自动保存计时器（每 5000ms 存一次，帧率无关）
const SAVE_INTERVAL_MS: number = 5000;
let saveAccumMs: number = 0;
// 恢复标记：从开始界面点"继续游戏"进入 loading，loading 完成时应用快照
let pendingRestore: boolean = false;
...
function _saveSnapshot(): void {
  ...
  if (curPhase !== PHASE_PLAY && curPhase !== PHASE_BOSS_WARNING &&
      curPhase !== PHASE_BOSS && curPhase !== PHASE_LEVEL_UP) return;
  const upState = getUpgradeSaveState();
  // BOSS 战中断时保留血量/阶段（预警阶段中断只存 bossPending，从预警重打）
  let bossState: { hp: number; maxHp: number; bossIndex: number; attackPhase: number } | null = null;
  ...
  saveGame(
    upState.weapons, upState.passives,
    curPhase === PHASE_BOSS || curPhase === PHASE_BOSS_WARNING,  // 恢复后回到 BOSS 战
    bossState,
...
function start(): void {
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "hidden") _saveSnapshot();
  });
}
    ...
    case PHASE_PLAY:
      ...
      if (hero) {
        updateAndDrawSpecialWeapons(
          ...
          (enemyId, factor, durationMs) => Enemy.applySlow(enemyId, factor, durationMs),
        );
      }
      drawScoreEffects();
      drawDamageEffects();
...
const isDevServer: boolean = document.querySelector('script[src^="/src/"]') !== null;
