...
function resolveTargetDelta(): number {
  try {
    const params = new URLSearchParams(typeof location !== "undefined" ? location.search : "");
    const fps = Number(params.get("fps"));
    if (fps > 0 && Number.isFinite(fps)) return 1000 / fps;
  } catch {
    // 非浏览器环境（测试）走默认值
  }
  return 1000 / 60;
...
export const TARGET_DELTA: number = resolveTargetDelta();
