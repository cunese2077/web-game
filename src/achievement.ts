...
// 成就分档等级
export type AchievementTier = 0 | 1 | 2 | 3; // 0=未解锁, 1=铜, 2=银, 3=金
...
interface DerivedStats {
  ...
  highestLevel: number;
  highestScore: number;
  hardGames: number;        // 困难模式通关次数
  noDamageGames: number;    // 无伤通关次数
  highestKills: number;     // 单局最高击杀
}

// 成就分档定义
interface AchievementTierDef {
  threshold: number;        // 达到此值解锁该档
  desc: TextKey;            // 该档描述
}
...
interface AchievementDef {
  id: string;
  label: TextKey;
  tiers: AchievementTierDef[];  // 按铜/银/金顺序，长度=3
  getStat: (stats: DerivedStats) => number;  // 获取当前统计值
}
...
let achievements: Record<string, number> = {};
...
interface LastGameData {
  ...
  damageTaken: number;
  newAchievementTiers: { id: string; tier: number }[];
}
...
function computeStats(): DerivedStats {
  ...
  for (const r of records) {
    ...
    if (r.kills > highestKills) highestKills = r.kills;
    if (r.difficulty === "hard") hardGames++;
    if (r.damageTaken === 0) noDamageGames++;
  }
...
function computeTier(def: AchievementDef, stats: DerivedStats): number {
  ...
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed.records)) {
        ...
        for (const r of records) {
          if (!r.difficulty) r.difficulty = "normal";
          if (r.damageTaken === undefined) r.damageTaken = -1; // 未知，不计入无伤
        }
...
function recordGameEnd(score: number, level: number, kills: number, bossKills: number, difficulty: Difficulty, damageTaken: number): string[] {
  ...
  // 检查新解锁的成就档位
  const stats = computeStats();
  const newTiers: { id: string; tier: number }[] = [];
  for (const def of ACHIEVEMENT_DEFS) {
...
function getAchievementTier(id: string): number {
