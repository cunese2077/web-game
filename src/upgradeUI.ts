...
// ========== 卡片布局计算 ==========
function calcLayout(): { cardW: number; cardH: number; gap: number; startX: number; startY: number } {
...
function _drawIcon(icon: string, cx: number, cy: number, color: string): number {
  ctx.save();
...
// ========== 圆角矩形路径 ==========
function _roundRect(x: number, y: number, w: number, h: number, r: number): void {
  ctx.beginPath();
  ctx.moveTo(x + r, y);
...
// ========== 自动换行文本 ==========
function _wrapText(text: string, x: number, y: number, maxWidth: number, lineHeight: number): void {
...
function handleUpgradeClick(clickX: number, clickY: number): "selected" | "selected_evolution" | "rerolled" | null {
  // 检查卡片点击
