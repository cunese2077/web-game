...
import {
  ...
  WINGMAN_INTERVAL_MS, WINGMAN_OFFSET, WINGMAN_BULLET_SPEED_PX_PER_SEC,
...
class WingmanBullet {
  ...
  init(x: number, y: number, damage: number): void {
    this.x = x;
    this.y = y;
    this.damage = damage;
...
const wingmanBulletPool = new ObjectPool<WingmanBullet>(() => new WingmanBullet(0, 0, 0));
let wingmanCooldownsMs: number[] = [];   // 动态长度，基于僚机数量（ms，帧率无关）
