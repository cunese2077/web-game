...
import type { Hero } from "./heroEntity.js";
import type { BuffState } from "./types.js";

// 模块级单例引用（Hero 构造时注册，事件回调与查询访问器依赖）
let activeHero: Hero | null = null;

function setActiveHero(hero: Hero | null): void {
  activeHero = hero;
}

function getActiveHero(): Hero | null {
  return activeHero;
}
...
function setPauseBtnArea(area: { x: number; y: number; w: number; h: number }): void {
  _pauseBtnArea = area;
}
...
function getSoundIconArea(): { x: number; y: number; w: number; h: number } {
