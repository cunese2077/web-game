...
#### [已修复] 移动端画布残影累积（子弹/敌机/战机残影不消失）

- **现象**：手机上游戏运行一段时间后，子弹在底部累积不消失，敌机疯狂渲染不消失，玩家战机重复渲染之前位置不消失。桌面端不复现。
- **根因分析**：`src/ui.ts` 的 `paintBg()` 中滚动偏移 `y` 的重置逻辑使用了严格相等比较 `y++ === height && (y = 0)`。当移动端画布尺寸缩小时（地址栏显示/隐藏、横竖屏切换触发 `visualViewport.resize`），若当前 `y` 已超过新的 `height`，`y` 永远不会再等于 `height`，导致 `y` 无限递增。当 `y > 2 * height` 时，两张 `drawImage(bg, 0, y, ...)` 和 `drawImage(bg, 0, y - height, ...)` 都画在画布可见区域之外，**画布完全不被背景覆盖**，每帧的游戏对象（子弹/敌机/战机）绘制叠加在旧帧上，形成残影累积。
- **触发时序**：游戏运行中 `y` 从 0 递增 → 地址栏变化触发 resize 使 `height` 缩小 → `y` 超过新 `height` 后失控 → 约 65 秒后（`y > 2 * height`）残影爆发式累积。
- **为什么只在移动端复现**：桌面端画布尺寸固定（`min(innerWidth, 480) × min(innerHeight, 800)`），不会缩小；移动端地址栏和横竖屏切换会导致 `visualViewport.height` 变化。
- **修复位置**：[src/ui.ts](web-game/src/ui.ts) 的 `paintBg()` 函数，将 `===` 改为 `>=`。
- **修复代码**：
  ```typescript
  // 修复前：y++ === height && (y = 0);
  // 修复后：
  ...
  if (y >= height) y = 0;
  ```
- **状态**：**已修复并验证**。手机访问 `192.168.43.23:8080` 测试通过。
- **经验**：任何滚动/循环重置逻辑中，将单调递增的计数器与可变的边界值比较时，必须使用 `>=`（或取模），绝不能用 `===`，因为边界值可能缩小到计数器之下。

#### [已修复] 开始界面改为 Canvas 多语言文本 + 动画

- **改动**：将开始界面从静态图片（start.png）改为 Canvas 绘制文本，支持 i18n 多语言。
- **新增 TextKey**：`start.title`（"飞机大战" / "PLANE WAR"）、`start.clickToStart`（"点击开始游戏" / "Click to Start"）。
- **动画效果**：
  - 标题：金色三段渐变（#ffe066 → #ffd700 → #ff8c00）+ 深色描边 + 橙色发光 + 上下浮动
  - 飞机装饰：玩家战机居中显示 + 蓝色发光 + 左右漂移动效（sin/cos X/Y 偏移）+ 摇晃旋转
  - 提示文字：alpha 闪烁效果（0~1 周期变化）
- **布局**（从上到下）：飞机装饰（`cy - 110*fontScale`）→ 标题（`cy + 5*fontScale`）→ 提示（`cy + 75*fontScale`）
- **修改文件**：`src/ui.ts`（paintLogo 重写）、`src/i18n.ts`（新增 TextKey）
- **logoFrame 防溢出**：使用 `(logoFrame + 1) % 10000` 取模，避免长时间停留开始界面导致数值过大影响三角函数精度
...
#### [已修复] 音效图标优化 + 点击导致战机移动 + 冗余变量清理 + 移动端热区扩大
